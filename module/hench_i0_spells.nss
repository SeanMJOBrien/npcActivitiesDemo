...
const int HENCH_AREA_CHECK_MIND_AND_FEAR    = 7;
...
int GetBestWeaponBuffTarget(int buffSelf, int buffOthers, int nItemProp, int nEnhanceLevel, int nFlags, int nSpell, int nSpellLevel)
{
    ...
    for (; curCount < nMaxCount; curCount++)
    {
        ...
        if (nEnhanceLevel != -1)
        {
            if (GetItemAttackBonus(oWeapon1) >= nEnhanceLevel)
            {
                continue;
            }
        }
...
//$INSERTIONPOINTSTART
        ...
        {
            bFoundSpellGlobal = TRUE;
            if (nLastSpellCast != FEAT_TURN_UNDEAD && allowAttackAbilities && nRange <= 0.0 + 20.0)
            {
        ...
        if (GetHasFixedSpell(SPELL_IMPLOSION))
        {
            ...
            {
                if (GetTotalEnemyCount(oTarget, 1,SHAPE_SPHERE, RADIUS_SIZE_MEDIUM, 8.0, 9, -1000, HENCH_CHECK_SR_FORT_SAVE, HENCH_AREA_NO_CHECK))
                {
                    nAreaPosition = 58;
        ...
        {
            if (nLastSpellCast != SPELL_BIGBYS_GRASPING_HAND && nRange <= 40.0 && useAttackSpells && CheckSpellSaves(oTarget, 7, HENCH_CHECK_SR_ONLY) && !GetHasSpellEffect(SPELL_BIGBYS_CLENCHED_FIST, oTarget))
            {
        ...
        if (GetHasSpell(SPELL_HENCH_Beholder_Anti_Magic_Cone))
        {
            bFoundSpellGlobal = TRUE;
            if (1 && (d100() <= 66))
            {
        ...
        {
            bFoundSpellGlobal = TRUE;
            if (nLastSpellCast != SPELLABILITY_BOLT_WEB && nRange <= 20.0 && allowAttackAbilities && !GetIsImmune(oTarget, IMMUNITY_TYPE_ENTANGLE, OBJECT_SELF) && !GetHasEffect(EFFECT_TYPE_ENTANGLE, oTarget)  && !GetCreatureFlag(oTarget, CREATURE_VAR_IS_INCORPOREAL))
            {
...
//    Jug_Debug(GetName(OBJECT_SELF) + " targetType " + IntToString(nTargetType) + " nSpell = " + IntToString(nSpell));
    ...
    else if (nTargetType == TARGET_SPELLABILITY_AREA_ON_OBJECT)
    {
        ...
        {
            ActionCastSpellAtObject(SPELL_HENCH_Beholder_Special_Spell_AI, oTarget,
