...
void main()
{
    ...
    InitializeItemSpells(bPolymorphed, HENCH_INIT_ALL_SPELLS);
    ...
    if (GetLocalInt(OBJECT_SELF, henchHealCountStr) == -1)
    {
        ...
        if(HenchTalentHeal(GetMaster(), bPolymorphed ? HENCH_HAS_POLYMORPH_EFFECT : 0, TRUE))
        {
            DelayCommand(2.0, VoiceCanDo());
            return;
        }
    ...
    if (GetIsObjectValid(oHealTarget))
    {
        if(HenchTalentHeal(oHealTarget, bPolymorphed ? HENCH_HAS_POLYMORPH_EFFECT : 0, TRUE))
        {
            SetLocalInt(OBJECT_SELF, henchHealCountStr, curHealCount + 1);
            return;
        }
    ...
    {
        if (HenchTalentHealAll(bPolymorphed ? HENCH_HAS_POLYMORPH_EFFECT : 0, TRUE))
        {
            SetLocalInt(OBJECT_SELF, henchHealCountStr, curHealCount + 1);
            return;
        }
