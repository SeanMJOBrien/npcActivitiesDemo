...
const int HENCH_HEALING_CURE_SPELL_START =          0x0001;
const int HENCH_HEALING_INFLICT_SPELL_START =       0x0100;

const int HENCH_HEALING_ALLOW_SPONTANEOUS_CURE =    0x20000;
const int HENCH_HEALING_ALLOW_SPONTANEOUS_INFLICT = 0x40000;
const int HENCH_HEALING_CHECK_CLERIC_SPONTANEOUS =  0x60000;

int gHealingInformation;
...
void InitializeItemSpells(int bPolymorphed, int nInitType)
{
    ...
    if (!bDisablingEffect && !bSpellCastingProblem && (nInitType == HENCH_INIT_ALL_SPELLS))
    {
        int bIAmCleric = GetLevelByClass(CLASS_TYPE_CLERIC) > 0;
        ...
        if (bIAmCleric)
        {
            ...
            {
                gHealingInformation = HENCH_HEALING_ALLOW_SPONTANEOUS_CURE;
            }
            else if (iAlignment == ALIGNMENT_EVIL)
            {
                gHealingInformation = HENCH_HEALING_ALLOW_SPONTANEOUS_INFLICT;
            }
            else
            {
            ...
            for (testIndex = SPELL_CURE_CRITICAL_WOUNDS /* 31 */; testIndex < 36; testIndex++)
            {
                ...
                {
                    gHealingInformation = gHealingInformation | bitSetPosition;
                }
                bitSetPosition *= 2;
            }
...
int GetHasCureSpell(int spellID)
{
    if (!onlyUseTalents)
    {
        if (gHealingInformation & HENCH_HEALING_ALLOW_SPONTANEOUS_INFLICT)
        {
            // cleric must have previously set bit array for cure
            if (gHealingInformation & (HENCH_HEALING_CURE_SPELL_START << (spellID - SPELL_CURE_CRITICAL_WOUNDS)))
            {
                bFoundSpellGlobal = TRUE;
                bFoundItemSpell = FALSE;
                bFoundPotionOnly = FALSE;
...
int GetHasInflictSpell(int spellID)
{
    if (!onlyUseTalents)
    {
        if (gHealingInformation & HENCH_HEALING_ALLOW_SPONTANEOUS_CURE)
        {
            // cleric must have previously set bit array for inflict
            if (gHealingInformation & (HENCH_HEALING_INFLICT_SPELL_START << (spellID - SPELL_INFLICT_MINOR_WOUNDS)))
            {
                bFoundSpellGlobal = TRUE;
                bFoundItemSpell = FALSE;
                bFoundPotionOnly = FALSE;
...
void ActionCastFixedSpellOnObject(int nSpell, object oTarget)
{
    ...
    if (gHealingInformation & HENCH_HEALING_CHECK_CLERIC_SPONTANEOUS)
    {
        switch(nSpell)
        {
        ...
        case SPELL_CURE_CRITICAL_WOUNDS:
            if (!(gHealingInformation & (HENCH_HEALING_CURE_SPELL_START << (nSpell - SPELL_CURE_CRITICAL_WOUNDS))))
            {
                bUseCheat = TRUE;
                nMainSpell = nSpell;
            }
        ...
        case SPELL_INFLICT_SERIOUS_WOUNDS:
            if (!(gHealingInformation & (HENCH_HEALING_INFLICT_SPELL_START << (nSpell - SPELL_INFLICT_MINOR_WOUNDS))))
            {
                bUseCheat = TRUE;
                nMainSpell = nSpell;
            }
