//HornOfRetreat - Assigns the guardcastle vTag to any creatuers caught in it's AoE
...
void main()
{
    ...
    switch (nEvent)
    {
        ...
        case X2_ITEM_EVENT_ACTIVATE:
            ...
            AssignCommand(oPC, ActionSpeakString("I am using my mighty horn!", TALKVOLUME_SHOUT));
            //Get the first target in the radius around the caster
            AssignCommand(oSpellTarget, ActionSpeakString("I am listening!", TALKVOLUME_SHOUT));
            oSpellTarget = GetFirstObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_MEDIUM, GetLocation(OBJECT_SELF));
            while(GetIsObjectValid(oSpellTarget))
            {
//                if(GetIsReactionTypeFriendly(oSpellTarget) || GetFactionEqual(oSpellTarget))
                AssignCommand(oSpellTarget, ActionSpeakString("I am friendly!", TALKVOLUME_SHOUT));
                {
                    // Set localInt "noheart" to 1 to block heartbeat for oTarget(s)
                    SetLocalInt(oSpellTarget, "noheart", 1);
                    AssignCommand(oSpellTarget, ActionSpeakString("localInt noheart is now 1", TALKVOLUME_SHOUT));
                }
                //Get the next target in the specified area around the caster
                oSpellTarget = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_COLOSSAL, GetLocation(OBJECT_SELF));
                AssignCommand(oSpellTarget, ActionSpeakString("Next!", TALKVOLUME_SHOUT));
            }
            break;
/*
