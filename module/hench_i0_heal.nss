...
//::////////////////////////////////////////////*/
int HenchTalentHeal(object oHeal, int iCurEffects, int bForce)
{
...
int HenchTalentHealAllies(int iCurEffects, int bForce)
{
...
//        Jug_Debug(GetName(OBJECT_SELF) + " is checking heal target " + GetName(oTarget));
    ...
    {
        if (HenchTalentHeal(oHeal, iCurEffects, bForce))
        {
            return TRUE;
        }
    }
    ...
    {
        if (HenchTalentHeal(oUndead, iCurEffects, bForce))
        {
            return TRUE;
        }
    }
...
// HEAL SELF & OTHERS
int HenchTalentHealAll(int iCurEffects, int bForce)
{
    object oRealMaster = GetRealMaster();

    if((GetAssociateHealMaster() || bForce) && GetObjectSeen(oRealMaster) &&
        (GetCurrentHitPoints(oRealMaster) > HENCH_BLEED_NEGHPS))
    {
        if (HenchTalentHeal(oRealMaster, iCurEffects, TRUE))
        {
            return TRUE;
        }
    }
    if (HenchTalentHeal(OBJECT_SELF, iCurEffects, bForce))
    {
        return TRUE;
    }
    return HenchTalentHealAllies(iCurEffects, bForce);
}
