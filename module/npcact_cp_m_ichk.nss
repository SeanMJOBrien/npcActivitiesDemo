///////////////////////////////////////////////////////////////////////
// npcact_cp_m_ichk - NPC ACTIVITIES 6.0 Professions Merchant
// Intimidation Check
// Store Amount of check missed by as
// nIntimidationCheck on PC.  If number is negative then that is how
...
void main()
{
    ...
    int nPDC=GetLocalInt(oMerchant,"nProfMerchBarterIDC");
    int nPSC=GetLocalInt(oMerchant,"nProfMerchBarterISC");
    int nPMF=GetLocalInt(oMerchant,"nProfMerchBarterIMF");
    int nPCF=GetLocalInt(oMerchant,"nProfMerchBarterICF");
    string sSay;
    string sPMF=GetLocalString(oMerchant,"sProfMerchBarterIMF");
    string sPCF=GetLocalString(oMerchant,"sProfMerchBarterICF");
    string sPYS=GetLocalString(oMerchant,"sProfMerchBarterIYS");
    int nDC;
    int nR;
    ...
    int nPCount=GetLocalInt(oMerchant,"nICount_"+sPID);
    int nMaxPrice;
    int nMinPrice;
    int nN;
    ...
    if (nPDC==0) nPDC=10;
    if (nPSC==0) nPSC=4+d4();
    nDC=GetSkillRank(SKILL_INTIMIDATE,oMerchant)+nPDC;
    ...
    sS=ColorRGBString("[intimidation attempt]",6,2,2);
    sS=sS+" ROLL: "+ColorRGBString(IntToString(nR),2,2,6);
    nDC=nDC-nR;
    SetLocalInt(oPC,"nIntimidationCheck",nDC);
    if (nDC<1)
    { // success
    ...
    nPCount++;
    SetLocalInt(oMerchant,"nICount_"+sPID,nPCount);
}
